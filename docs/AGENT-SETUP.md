...
| `wdkwhere` works in your shell but the service log says NOT FOUND | The service runs as **LocalSystem**, whose PATH is not your user PATH. `dotnet tool install --global` puts tools under `%USERPROFILE%\.dotnet\tools`. With interactive signing (`Auto` / `InteractiveUser`), the agent probes the active console user's PATH, that `.dotnet\tools` folder, and Windows SDK `bin\<ver>\x64` directories. Or set `--signtool` / `SignToolPath` to an absolute path. Note: `status` uses a **combined probe** (calling console PATH plus, for non-`SameProcess` modes, active-console-user directories and Windows SDK bins), which can succeed even when LocalSystem's own PATH alone would not. |
